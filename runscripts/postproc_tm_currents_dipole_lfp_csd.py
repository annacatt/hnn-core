...
def collect_intrinsic_sources( # capacitive + ionic OR agg_i_mem
    net,
    trial_idx: int = 0,
...
def _z_edges_from_array(net, array_name):
    """Build z_edges centered on each contact (height = contact spacing)."""
    array = net.rec_arrays[array_name]
    z = np.asarray(array.positions)[:, 2]
    delta = float(np.abs(z[1] - z[0]))
    edges = np.concatenate([z - delta / 2.0, [z[-1] + delta / 2.0]])
    return edges

def _network_extent_um(net):
    """Rectangular lateral extent (width_x, width_y) of the network in μm.

    Computed from net._inplane_distance x (net._N_pyr_x - 1, net._N_pyr_y - 1) [μm].
    """
    inplane = getattr(net, "_inplane_distance") #float(getattr(net, "_inplane_distance", 1.0))
    
    width_x = (net._N_pyr_x - 1) * inplane
    width_y = (net._N_pyr_y - 1) * inplane
    return width_x, width_y


# sources is a list where each entry represents one segment of one section of one cell for one channel (could also be agg_i_mem though)
# segment_x is the normalized position along the section (0 to 1) where the source is located, used to interpolate the z value of the source from the section endpoints and get the actual z-coordinate of that segment
def build_binning_matrix_for_sources(
    net,
    sources: Sequence[SourceInfo],
    z_edges: np.ndarray | None = None,
    array_name: str | None = None,
):
    """Build (n_bins x n_sources) z-bin matrix.

    Each source is assigned to exactly one z-bin (no xy filtering); all
    segments in a slab contribute to that slab's CSD regardless of xy.
    The slab volume is taken from the network's lateral extent times the
    bin height.

    Parameters
    ----------
    z_edges : ndarray (n_bins+1,) or None
        Bin edges along z (μm). If None, derived from `array_name`.
    array_name : str or None
        If given and z_edges is None, build z_edges centered on each contact
        of net.rec_arrays[array_name] with height = contact spacing.

    Returns
    -------
    B : ndarray, shape (n_bins, n_sources), {0, 1}
    V_bin : ndarray, shape (n_bins,)   (mm^3)
    z_centers : ndarray, shape (n_bins,)   (μm)
    """
    if z_edges is None:
        if array_name is None:
            raise ValueError("Pass either z_edges or array_name.")
        z_edges = _z_edges_from_array(net, array_name)
    z_edges = np.asarray(z_edges, dtype=float)
    n_bins = len(z_edges) - 1

    network_extent = _network_extent_um(net)         # (width_x_μm, width_y_μm)

    # AC:
    # e.g.: template = net.cell_types["L5_pyramidal"]["cell_object"]
    # what sections are in the template cell? print(list(template.sections.keys()))
    # sec = template.sections["apical_trunk"]
    # print(sec.end_pts)  # [[x0, y0, z0], [x1, y1, z1]]
    # sec.end_pts stores coordinates relative to the soma
    # sec_start = end_pts[0] + soma_pos   # absolute [x, y, z] of section start
    # sec_end   = end_pts[1] + soma_pos   # absolute [x, y, z] of section end
    template_cells = {
        ct: net.cell_types[ct]["cell_object"]
        for ct in sorted({src.cell_type for src in sources})
    }

    # Cache (sec_start, sec_end) per (cell_type, gid, section) — same convention
    # as build_transfer_resistance_matrix_for_sources. All segments of one
    # section share the same endpoints; we interpolate z per segment outside
    # the cache lookup.
    geom_cache: dict = {}

    B = np.zeros((n_bins, len(sources)), dtype=float)
    for col in range(len(sources)):
        src = sources[col]
        key = (src.cell_type, src.gid, src.section)
        if key not in geom_cache:
            sec_start, sec_end, _, _ = _get_global_section_geometry(
                net, template_cells, src
            )
            geom_cache[key] = (sec_start, sec_end)
        sec_start, sec_end = geom_cache[key]
        z = sec_start[2] + src.segment_x * (sec_end[2] - sec_start[2]) # linear interpolation between the section endpoints to get the z value of the source segment

        # np.digitize: index of bin such that edges[idx-1] <= z < edges[idx]
        idx = int(np.digitize(z, z_edges)) - 1
        if 0 <= idx < n_bins:
            B[idx, col] = 1.0

    # Slab volumes (mm^3): rectangular cross-section x bin height.
    h_um = np.diff(z_edges)[0] # they are all identical                          # μm
    area_mm2 = (network_extent[0] / 1000.0) * (network_extent[1] / 1000.0)
    V_bin = area_mm2 * (h_um / 1000.0)               # mm^3

    z_centers = 0.5 * (z_edges[:-1] + z_edges[1:])
    return B, V_bin, z_centers

def compute_csd_from_sources(
    B: np.ndarray,
    current_matrix_nA: np.ndarray,
    V_bin: np.ndarray,
) -> np.ndarray:
    """Return CSD (μA / mm^3), shape (n_bins, n_times).

        CSD[k, t] = (B @ I_nA)[k, t] * 1e-3 / V_bin

    1e-3 converts nA to μA. V_bin in mm^3.
    """
    if B.shape[1] != current_matrix_nA.shape[0]:
        raise ValueError(
            "B columns must match I_nA rows: "
            f"{B.shape[1]} vs {current_matrix_nA.shape[0]}"
        )
